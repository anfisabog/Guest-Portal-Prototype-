...
const AvatarOnlineIndicator = ({ size, status, className }) => (jsx("span", { className: cx("absolute right-0 bottom-0 rounded-full ring-[1.5px] ring-bg-primary", status === "online" ? "bg-fg-success-secondary" : "bg-fg-disabled_subtle", sizes$1[size], className) }));
...
const Avatar = ({ contrastBorder = true, size = "md", src, alt, initials, placeholder, placeholderIcon: PlaceholderIcon, badge, status, verified, focusable = false, className, }) => {
    ...
    const renderMainContent = () => {
        if (src && !isFailed) {
            return jsx("img", { "data-avatar-img": true, className: "size-full rounded-full object-cover", src: src, alt: alt, onError: () => setIsFailed(true) });
        }
        if (initials) {
    ...
    const renderBadgeContent = () => {
        if (status) {
            return jsx(AvatarOnlineIndicator, { status: status, size: size === "xxs" ? "xs" : size });
        }
        if (verified) {
            return (jsx(VerifiedTick, { size: size === "xxs" ? "xs" : size, className: cx("absolute right-0 bottom-0", (size === "xxs" || size === "xs") && "-right-px -bottom-px") }));
        }
        return badge;
    };
    return (jsxs("div", { "data-avatar": true, className: cx("relative inline-flex shrink-0 items-center justify-center rounded-full bg-avatar-bg outline-transparent", 
        // Focus styles
...
export { Avatar as A, VerifiedTick as V, AvatarOnlineIndicator as a };
//# sourceMappingURL=avatar-DeJPrYaB.esm.js.map
