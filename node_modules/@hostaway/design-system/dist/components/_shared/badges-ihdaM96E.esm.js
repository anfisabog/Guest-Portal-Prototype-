...
import { D as Dot } from './dot-icon-DOJ8mWXT.esm.js';
...
const Badge = (props) => {
    const { type = 'pill-color', color = 'gray', children, className, ...rest } = props;
    const colors = withPillTypes[type];
    const pillStyles = 'py-0.5 px-2 text-xs font-medium';
    const badgeStyles = 'py-0.5 px-1.5 text-xs font-medium';
    const sizes = {
        [badgeTypes.pillColor]: pillStyles,
        [badgeTypes.badgeColor]: badgeStyles,
        [badgeTypes.badgeModern]: badgeStyles,
    };
    return (jsx("span", { className: cx(colors.common, sizes[type], colors.styles[color].root, className), ...rest, children: children }));
};
const BadgeWithDot = (props) => {
    const { color = 'gray', type = 'pill-color', className, children, ...rest } = props;
    const colors = withBadgeTypes[type];
    const pillStyles = 'gap-1 py-0.5 pl-1.5 pr-2 text-xs font-medium';
    const badgeStyles = 'gap-1 py-0.5 px-1.5 text-xs font-medium';
    const sizes = {
        [badgeTypes.pillColor]: pillStyles,
        [badgeTypes.badgeColor]: badgeStyles,
        [badgeTypes.badgeModern]: badgeStyles,
    };
    return (jsxs("span", { className: cx(colors.common, sizes[type], colors.styles[color].root, className), ...rest, children: [jsx(Dot, { className: colors.styles[color].addon, size: "sm" }), children] }));
};
const BadgeWithIcon = (props) => {
    const { color = 'gray', type = 'pill-color', iconLeading: IconLeading, iconTrailing: IconTrailing, children, className, } = props;
    const colors = withBadgeTypes[type];
    ...
    return (jsxs("span", { className: cx(colors.common, sizes[type][icon], colors.styles[color].root, className), children: [IconLeading && (jsx(IconLeading, { className: cx(colors.styles[color].addon, 'size-3 stroke-3') })), children, IconTrailing && (jsx(IconTrailing, { className: cx(colors.styles[color].addon, 'size-3 stroke-3') }))] }));
};

export { BadgeWithDot as B, BadgeWithIcon as a, Badge as b };
//# sourceMappingURL=badges-ihdaM96E.esm.js.map
