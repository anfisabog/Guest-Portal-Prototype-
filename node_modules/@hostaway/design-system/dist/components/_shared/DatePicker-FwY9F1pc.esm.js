...
import { $ as $458b0a5536c1a7cf$export$40bfa8c7b0832715 } from './useControlledState-mV1mviQ6.esm.js';
import { ChevronLeft, ChevronRight, Calendar as Calendar$2 } from '@untitledui/icons';
import { useLocale, useSlottedContext, RangeCalendarContext, CalendarCell as CalendarCell$1, DateInput as DateInput$1, DateSegment, CalendarContext, Calendar as Calendar$1, Heading, CalendarGrid, CalendarGridHeader, CalendarHeaderCell, CalendarGridBody, CalendarStateContext, DatePicker as DatePicker$1, Group, Popover, Dialog } from 'react-aria-components';
import { c as cx } from './cx-DnIR8RkH.esm.js';
import { B as Button } from './BaseButton-CTtHjEZn.esm.js';
import { $ as $18f2051aff69b9bf$export$43bb16f9c6d9e3f7 } from './context-DUiWIuVv.esm.js';
import { F as FieldWrapper } from './FieldWrapper-UtX7P_J-.esm.js';
...
const CalendarCell = ({ date, isHighlighted, ...props }) => {
    var _a, _b;
    ...
    const start = (_a = rangeCalendarContext === null || rangeCalendarContext === void 0 ? void 0 : rangeCalendarContext.value) === null || _a === void 0 ? void 0 : _a.start;
    const end = (_b = rangeCalendarContext === null || rangeCalendarContext === void 0 ? void 0 : rangeCalendarContext.value) === null || _b === void 0 ? void 0 : _b.end;
    ...
    const isTodayDate = $14e0f24ef4ac5c92$export$629b0a497aa65267(date, $14e0f24ef4ac5c92$export$aa8b41735afcabd2());
    return (jsx(CalendarCell$1, { ...props, date: date, className: ({ isDisabled, isFocusVisible, isSelectionStart, isSelectionEnd, isSelected, isOutsideMonth }) => {
            const isRoundedLeft = isSelectionStart || dayOfWeek === 0;
            const isRoundedRight = isSelectionEnd || dayOfWeek === 6;
            return cx("relative size-10 focus:outline-none", isRoundedLeft && "rounded-l-full", isRoundedRight && "rounded-r-full", isInRange && isDisabled && "bg-active", isSelected && isRangeCalendar && "bg-active", isDisabled ? "pointer-events-none" : "cursor-pointer", isFocusVisible ? "z-10" : "z-0", isRangeCalendar && isOutsideMonth && "hidden", 
            ...
            return (jsxs("div", { className: cx("relative flex size-full items-center justify-center rounded-full text-sm", 
                // Disabled state.
                ...
                !isSelected && !isDisabled ? "hover:bg-primary_hover hover:font-medium!" : "", !isSelected && isTodayDate ? "bg-active font-medium hover:bg-secondary_hover" : ""), children: [formattedDate, (isHighlighted || isTodayDate) && (jsx("div", { className: cx("absolute bottom-1 left-1/2 size-1.25 -translate-x-1/2 rounded-full", isDisabled ? "bg-fg-disabled" : markedAsSelected ? "bg-fg-white" : "bg-fg-brand-primary") }))] }));
        } }));
};

const DateInput = (props) => {
    return (jsx(DateInput$1, { ...props, className: cx("flex rounded-lg bg-primary px-2.5 py-2 text-md shadow-xs ring-1 ring-primary ring-inset focus-within:ring-2 focus-within:ring-brand", typeof props.className === "string" && props.className), children: (segment) => (jsx(DateSegment, { segment: segment, className: cx("rounded px-0.5 text-primary tabular-nums caret-transparent focus:bg-brand-solid focus:font-medium focus:text-white focus:outline-hidden", 
...
const CalendarContextProvider = ({ children }) => {
    ...
    return jsx(CalendarContext.Provider, { value: { value, onChange, focusedValue, onFocusChange }, children: children });
};
const PresetButton = ({ value, children, ...props }) => {
    ...
    return (jsx(Button, { ...props, 
        ...
        slot: null, size: "sm", color: "secondary", onClick: handleClick, children: children }));
};
const Calendar = ({ highlightedDates, className, 'data-id': dataId, ...props }) => {
    const context = useSlottedContext(CalendarContext);
    const ContextWrapper = context ? Fragment : CalendarContextProvider;
    return (jsx(ContextWrapper, { children: jsxs(Calendar$1, { ...props, className: (state) => cx("flex flex-col gap-3", typeof className === "function" ? className(state) : className), children: [jsxs("header", { className: "flex items-center justify-between", children: [jsx(Button, { slot: "previous", iconLeading: ChevronLeft, size: "sm", color: "tertiary", className: "size-8", "data-id": `${dataId}-prev-button` }), jsx(Heading, { className: "text-sm font-semibold text-fg-secondary" }), jsx(Button, { slot: "next", iconLeading: ChevronRight, size: "sm", color: "tertiary", className: "size-8", "data-id": `${dataId}-next-button` })] }), jsxs("div", { className: "flex gap-3", children: [jsx(DateInput, { className: "flex-1", "data-id": `${dataId}-date-input` }), jsx(PresetButton, { value: $14e0f24ef4ac5c92$export$d0bdf45af03a6ea3($14e0f24ef4ac5c92$export$aa8b41735afcabd2()), "data-id": `${dataId}-today-button`, children: "Today" })] }), jsxs(CalendarGrid, { weekdayStyle: "short", className: "w-max", children: [jsx(CalendarGridHeader, { className: "border-b-4 border-transparent", children: (day) => (jsx(CalendarHeaderCell, { className: "p-0", children: jsx("div", { className: "flex size-10 items-center justify-center text-sm font-medium text-secondary", children: day.slice(0, 2) }) })) }), jsx(CalendarGridBody, { className: "[&_td]:p-0 [&_tr]:border-b-4 [&_tr]:border-transparent [&_tr:last-of-type]:border-none", children: (date) => (jsx(CalendarCell, { date: date, isHighlighted: highlightedDates === null || highlightedDates === void 0 ? void 0 : highlightedDates.some((highlightedDate) => date.compare(highlightedDate) === 0), "data-id": `${dataId}-calendar-cell-${date}` })) })] })] }) }));
};
...
const DatePicker = ({ value: valueProp, defaultValue, onChange, onApply, onCancel, formatDate, t: textsProp, timeZone, label, hint, isRequired = false, isInvalid = false, 'data-id': dataId, fieldSize = 'auto', size, labelPosition = 'left', tooltip, description, id, ...props }) => {
    const texts = textsProp !== null && textsProp !== void 0 ? textsProp : DEFAULT_TEXTS;
    const tz = timeZone !== null && timeZone !== void 0 ? timeZone : $14e0f24ef4ac5c92$export$aa8b41735afcabd2();
    const formatter = $896ba0a80a8f4d36$export$85fd5fdf27bacc79({
        month: 'short',
        day: 'numeric',
        year: 'numeric',
    });
    const [value, setValue] = $458b0a5536c1a7cf$export$40bfa8c7b0832715(valueProp, defaultValue || null, onChange);
    const highlightedDates = useMemo(() => [$14e0f24ef4ac5c92$export$d0bdf45af03a6ea3(tz)], [tz]);
    const formatDisplayDate = (date) => {
        const jsDate = date.toDate(tz);
        return formatDate ? formatDate(jsDate) : formatter.format(jsDate);
    };
    const formattedDate = value ? formatDisplayDate(value) : texts.placeholder;
    return (jsx(FieldWrapper, { isRequired: isRequired, label: label, hint: hint, isInvalid: isInvalid, fieldSize: fieldSize, labelPosition: labelPosition, tooltip: tooltip, description: description, id: id, children: jsxs(DatePicker$1, { shouldCloseOnSelect: false, ...props, value: value, onChange: setValue, children: [jsx(Group, { children: jsx(Button$1, { size: "sm", color: "secondary", className: cx(isInvalid && 'ring-error_subtle', (fieldSize === 'full' || size === 'full') && sizeStyles.full), iconLeading: Calendar$2, "data-id": `${dataId}-calendar-icon-button`, children: formattedDate }) }), jsx(Popover, { offset: 8, placement: "bottom right", className: ({ isEntering, isExiting }) => cx('will-change-transform', isEntering &&
                        'duration-150 ease-out animate-in fade-in placement-right:origin-left placement-right:slide-in-from-left-0.5 placement-top:origin-bottom placement-top:slide-in-from-bottom-0.5 placement-bottom:origin-top placement-bottom:slide-in-from-top-0.5', isExiting &&
                        'duration-100 ease-in animate-out fade-out placement-right:origin-left placement-right:slide-out-to-left-0.5 placement-top:origin-bottom placement-top:slide-out-to-bottom-0.5 placement-bottom:origin-top placement-bottom:slide-out-to-top-0.5'), children: jsx(Dialog, { className: "rounded-2xl bg-primary shadow-xl ring ring-secondary_alt", children: ({ close }) => (jsxs(Fragment$1, { children: [jsx("div", { className: "flex px-6 py-5", children: jsx(Calendar, { highlightedDates: highlightedDates, "data-id": `${dataId}-calendar` }) }), jsxs("div", { className: "grid grid-cols-2 gap-3 border-t border-secondary p-4", children: [jsx(Button$1, { size: "sm", color: "secondary", onClick: () => {
                                                onCancel === null || onCancel === void 0 ? void 0 : onCancel();
                                                close();
                                            }, "data-id": `${dataId}-cancel-button`, children: texts.cancelBtn }), jsx(Button$1, { size: "sm", color: "primary", onClick: () => {
                                                onApply === null || onApply === void 0 ? void 0 : onApply();
                                                close();
                                            }, "data-id": `${dataId}-apply-button`, children: texts.applyBtn })] })] })) }) })] }) }));
};

export { $896ba0a80a8f4d36$export$85fd5fdf27bacc79 as $, Calendar as C, DatePicker as D, DateInput as a, CalendarCell as b, DEFAULT_TEXTS as c, $14e0f24ef4ac5c92$export$aa8b41735afcabd2 as d, $14e0f24ef4ac5c92$export$d0bdf45af03a6ea3 as e, $14e0f24ef4ac5c92$export$a2258d9c4118825c as f, $14e0f24ef4ac5c92$export$a5a3b454ada2268e as g, $14e0f24ef4ac5c92$export$ef8b6d9133084f4e as h, $14e0f24ef4ac5c92$export$42c81a444fbfb5d4 as i, $fae977aafc393c5c$export$6b862160d295c8e as j };
//# sourceMappingURL=DatePicker-FwY9F1pc.esm.js.map
