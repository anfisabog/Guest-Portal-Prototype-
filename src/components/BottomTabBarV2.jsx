...
// V2 — pill-style tab bar (Hostaway app style)
// • 16px side margins, rounded-2xl container
// • Active tab: full-height rounded pill background
// • Gradient fade above so scrolling content softly disappears beneath
...
// Total reserved space at bottom = gradient (32px) + tab bar (72px) = 104px
// Scrollable content uses pb-[104px] when this variant is active
export const TAB_BAR_V2_HEIGHT = 104

export default function BottomTabBarV2({ activeTab, onTabChange, navigate, cartCount = 0 }) {
  const tabs = [
    { key: 'home',    label: 'Home',    icon: (a) => <HomeIcon active={a} />,    onPress: () => { onTabChange('home');    navigate(SCREENS.HOMEPAGE)      } },
    { key: 'access',  label: 'Access',  icon: (a) => <AccessIcon active={a} />,  onPress: () => { onTabChange('access');  navigate(SCREENS.ACCESS_REVEAL) } },
    { key: 'upsells', label: 'Upsells', icon: (a) => <UpsellsIcon active={a} />, onPress: () => { onTabChange('upsells'); navigate(SCREENS.UPSELLS)       } },
  ]

  return (
    <div className="fixed bottom-0 pointer-events-none z-50" style={{ height: TAB_BAR_V2_HEIGHT, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 900 }}>

      {/* Gradient fade — smooth transition from scroll content into tab bar */}
      <div
        className="absolute top-0 left-0 right-0 h-10"
        style={{ background: 'linear-gradient(to bottom, transparent, var(--color-bg-warm))' }}
      />
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col items-center gap-0.5">
                    ...
                    <span className={`text-[11px] font-semibold leading-tight ${active ? 'text-(--color-hostaway-secondary-600)' : 'text-(--color-gray-600)'}`}>
                      {label}
                    </span>
                  </div>
                </button>
